...
extern enum WINDOW_FUNC_TYPE PS_simple_window_func; //简单PS方法中窗口函数类型
...
extern arb_t Narrow_up_step_beta; // 有限宽 upward step 模型用
extern arb_t Narrow_up_step_kappa;
extern arb_t Narrow_up_step_g;
extern arb_t Narrow_up_step_gamma;
extern arb_t Narrow_up_step_omega;
extern arb_t Narrow_up_step_A;
extern arb_t Narrow_up_step_cutoff_1;
...
extern arb_t PS_Root_zeta_to_zeta_G_min;//计算概率 P(ζ), 需要反解 ζ= F(ζ_G)，有时反函数的解析表达式不能写出，且反函数不是单值
extern arb_t PS_Root_zeta_to_zeta_G_max;
extern arb_t PS_Root_zeta_to_zeta_G_precision;
extern slong PS_Root_zeta_to_zeta_G_num;
...
extern arb_t PS_M_ratio_min; // I 型扰动质量比的最小值
extern arb_t PT_M_ratio_min;
extern arb_t PS_C_th; // 压缩函数的临界值 C_th
extern arb_t PS_C_l_th; // 线性压缩函数的临界值 C_th(r)
extern arb_t PS_zeta_th; // 曲率扰动估算阈值 ζ_th
extern arb_t PS_delta_th; // 密度扰动估算阈值 δ_th
extern arb_t PS_abundance_int_precision; //计算丰度β和f的积分精度
extern arb_t PS_abundance_simple_int_precision;
extern arb_t PS_abundance_simple_int_min;
extern arb_t PS_abundance_simple_int_max;

extern arb_t Continuum_spectrum_A_old; //连续谱计算相关
extern arb_t Delta_spectrum_x_m; //求连续谱的特征模式用
extern arb_t Delta_spectrum_reenter_coefficient_C; //求连续谱的特征模式用
extern arb_t Continuum_spectrum_k_ch; //连续谱的特征模式
extern arb_t Continuum_spectrum_k_ch_times_r_m; //k_ch*r_m
extern enum GET_K_CH_TYPE Get_k_ch_type; //求连续谱特征模式方法


//非高斯情况下，利用非高斯功率谱对 typical profile 修正用
extern bool Non_Gaussian_typical_profile_correction;
