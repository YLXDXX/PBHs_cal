...
int interior_GW_I_c_func(arb_t res, const arb_t tau, void* param, const slong order, slong prec)
{
...
int interior_GW_I_s_func(arb_t res, const arb_t tau, void* param, const slong order, slong prec)
{
    arb_t s,t,w,x_tau,y_tau,T_x_tau,T_y_tau,T_x_tau_prime,T_y_tau_prime;
    arb_init(s);
    arb_init(t);
...
//解析函数表达式
void interior_GW_I_c_func_analyze(arb_t res, const arb_t d, const arb_t s, const arb_t tau, slong prec)
{
...
void interior_GW_I_s_func_analyze(arb_t res, const arb_t d, const arb_t s, const arb_t tau, slong prec)
{
    arb_t z,t,w,q,d_2,s_2,tau_2,d_tau,s_tau,A_s,A_d,B_s,B_d,C_d_s,C_s_d;
    arb_t sin_tau,sin_d_tau,sin_s_tau,cos_tau,cos_d_tau,cos_s_tau;
...
//取 x --> +∞
int I_func_v_u_x_limited(arb_t res, const arb_t v, const arb_t u, slong prec)
{
    return 0;
}
...
//取 x --> +∞ 且平方后取平均
int I_func_square_v_u_x_limited_average(arb_t res, const arb_t v, const arb_t u, const arb_t x, slong prec)
{
    arb_t s,t,w,z,r;
    arb_init(s);
    arb_init(t);
    ...
    //辐射时代 w=1/3
    //函数形式$=\frac{1}{2}\left(\frac{3(u^2 + v^2 - 3)}{4u^3v^3x}\right)^2\left(\left( - 4uv + (u^2 + v^2 - 3)\log\left|\frac{3 - (u + v)^2}{3 - (u - v)^2}\right|\right)^2 + \pi^2(u^2 + v^2 - 3)^2\Theta(v + u - \sqrt{3}) \right) $
    
    //左边系数 1/2 * [ 3(u^2+v^2-3)/(4*u^3*v^3*x) ]^2
    arb_one(s);
    ...
    arb_sqr(t,v,prec);
    arb_add(s,s,t,prec);
    arb_sub_ui(z,s,3,prec); // z=u^2+v^2-3
    arb_mul_ui(s,z,3,prec);
    
    arb_pow_ui(t,u,3,prec);//分母
    arb_pow_ui(w,v,3,prec);
    arb_mul(t,t,w,prec);
    ...
    //右边大括号部分：左边 [ -4uv + (u^2+v^2-3)*log|(3-(u+v)^2)/(3-(u-v)^2)| ]^2
    ...
    //右边大括号部分：右边 π^2 * (u^2+v^2-3)^2 * Θ(v+u-sqrt(3))
    arb_const_pi(w,prec);
    arb_sqr(w,w,prec);
    arb_sqr(z,z,prec);
    arb_mul(w,w,z,prec);
    ...
    arb_one(z);
    arb_mul_ui(z,z,3,prec);
    arb_sqrt(z,z,prec);
    arb_neg(z,z);
    arb_add(z,z,u,prec);
    arb_add(z,z,v,prec);
    
    Heaviside_Theta_function(r,z,prec);
