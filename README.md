...
而 `main.c`​ 中的相关计算，又是调用如下八个模块来完成的「在 `src/func`​ 下」：
...
> 注意，八个大模块中各小模块的顺序是按该模块的实际计算顺序排列
>
...
### Power_Spectra_numeric

包含如下：

`Inflation_odes_slove.h` 数值计算暴胀产生的功率谱
...
`ODEs.h` 常微分方程组求解算法，有三种方法可用：RFK45、DOPRI54.、DOP853
...
`fitting_DOF.h` 获取不同温度 $T$ 或不同波数 $k$ 对应的自由度数，波数 $k$ 对应的自由度数等

`output.h`​ 用于获取存储路径、输出数据的进度条展示，输出矢量对 $\bm{x}$ 和 $\bm{y}$ 到文件等

`other.h` 诱导引力波频率 $f$ 与波数 $k$ 的转换，在给定区间内产生线性或对数间隔等
...
此程序的编写中， [mxhphy](https://github.com/mxhphy) 学长给予了大量的指导，而其中的功率谱数值计算模块 `Power_Spectra_numeric` 则是直接源自于其编写的 Python 程序，于此特别鸣射
